#!/usr/bin/env python3
"""macOS DMG builder for Calendifier.
...
BUNDLE_ID = "uk.codecrafter.Calendifier"
FINAL_DMG = "calendifier.dmg"
RW_DMG = "_calendifier_rw.dmg"
VOLUME_NAME = f"Install {APP_NAME}"

# Source 1024x1024 master icon at the repo root.
SOURCE_PNG = "calendifier.png"

# Dark background matching Calendifier's dark theme (clock-face grey).
ICON_BG = (0x2D, 0x2D, 0x2D)

# App data folders shipped beside the code (source:dest-in-bundle).
DATA_DIRS = (
...
# Minimal hardened-runtime entitlements. Calendifier is a local-first app;
# disable-library-validation lets the hardened runtime load the
...
def build_app_bundle(entitlements_path: Path, icns_path: Path | None = None) -> Path:
    ...
    cmd = [
        ...
        *icon_args,
        # Pull in every app submodule plus the holiday data tables.
        "--collect-submodules=calendar_app",
...
def strip_build_artifacts(app_path: Path) -> None:
    ...
    # codesign --deep silently skips them but Gatekeeper flags them as unsigned,
    # rejecting the whole bundle. Remove them before signing.
    removed = 0
...
def create_dmg(app_path: Path) -> None:
    ...
    if dest.exists():
        shutil.rmtree(dest)
    # ditto preserves the framework symlinks macOS relies on; dereferencing them
    # invalidates embedded signatures and breaks dlopen at runtime.
