...
from installer.ops.progress import emit_progress as _progress
...
from calendar_app.shared.resources import find_app_icon_path
from version import APP_NAME, APP_AUTHOR, __version__
...
def _installer_staging_root() -> Path:
    ...
    return Path(local) / "CalendifierInstaller" / "staging"
...
def _register_uninstall(
    ...
    *,
    ...
    for ico_name in ["calendifier.ico", "Calendifier.ico"]:
...
def _deploy_runtime_icon_assets(*, install_dir: Path) -> None:
    """Deploy icon assets next to Calendifier.exe.
    ...
    if ico is not None:
        try:
            shutil.copy2(ico, install_dir / "calendifier.ico")
        except Exception:
            pass
...
def install_new(
    ...
    *,
    ...
    staging_dir = target_dir.parent / f".calendifier_staging.install.{uuid.uuid4().hex}"
...
def upgrade_or_reinstall(
    ...
    *,
    ...
    exe = current_install_dir / "Calendifier.exe"
    ...
    staging_dir = target_dir.parent / f".calendifier_staging.upgrade.{uuid.uuid4().hex}"
    if staging_dir.exists():
        shutil.rmtree(staging_dir, ignore_errors=True)
