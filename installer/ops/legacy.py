...
The app was renamed from "Calendifier" to "Calendifier". That rename changed
...
def migrate_legacy_appdata_dirs() -> None:
    """Move per-user data/cache dirs from the old "Calendifier" app name.
...
def cleanup_orphaned_legacy_install(target_dir: Path) -> None:
    ...
    Renaming the app from "Calendifier" to "Calendifier" also moved the default
    install directory from ``%LOCALAPPDATA%\\Calendifier`` to
    ``%LOCALAPPDATA%\\Calendifier``. The uninstall registry key, Start Menu
